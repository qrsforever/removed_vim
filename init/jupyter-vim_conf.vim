...
"""               return
"""           elseif a:flag == 2 " update
"""               let bufid = bufnr('__jupyter_term__')
"""               if bufid != -1
"""                   exec 'bunload! ' . bufid
"""               endif
"""           elseif a:flag == 0 " clear
"""               exec 'bwipeout __jupyter_term__'
"""           else
"""               exec 'bwipeout __jupyter_term__'
"""           endif
...
"""   nnoremap <unique> <silent> <leader>jw :call <SID>DoUpdateWindow(3)<CR> "open window
"""
...
" 1. pip3 install jupyter matplotlib PySide6
...
function s:TimerJupyterConnect(timer) abort
    exec 'silent! JupyterConnect'
endfunction

function! s:DoCreateAndConnect()
    exec 'silent! JupyterTerminateKernel'
    exec 'silent! !' . g:VIM_HOME . '/bin/0jupyter-qtconsole.sh'
    call timer_start(3500, function('s:TimerJupyterConnect'))
endfunction
...
nnoremap <unique> <silent> <leader>je :JupyterSendCell<CR>
nmap     <unique> <silent> <leader>js <Plug>JupyterRunTextObj
vmap     <unique> <silent> <leader>js <Plug>JupyterRunVisual
nnoremap <unique> <silent> <leader>jc :call <SID>DoCommand("c")<CR>
nnoremap <unique> <silent> <leader>jt :call <SID>DoCommand("t")<CR>
nnoremap <unique> <silent> <leader>jp :call <SID>DoCommand("p")<CR>
