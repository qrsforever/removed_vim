...
function! s:DoLeaderfFileWithPath(cwd)
    ...
    call inputsave()
    let dir = input("DIR@", dir, "dir")
    call inputrestore()
    if isdirectory(dir)
        exec 'Leaderf file --no-ignore --regexMode ' . dir
    endif
endfunc
...
function! s:DoLeaderfRgWithPath(cwd, nwrap, cbuf, icase, append)
    ...
    elseif a:cbuf == 2  " gG
        ...
        call inputsave()
        let dirstr = input("DIR@", getcwd(), "dir")
        call inputrestore()
        if dirstr == ""
            return
        endif
...
nnoremap <unique> <silent> sc :<C-U>Leaderf! cmdHistory<CR>
nnoremap <unique> <silent> sh :<C-U>Leaderf! searchHistory<CR>
nnoremap <unique> <silent> sj :<C-U>Leaderf! jumps<CR>
