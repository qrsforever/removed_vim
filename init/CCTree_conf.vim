...
func! CCTreeOpenFile(flag, cmd) "{{{
    ...
    else " a:flag == 1, a:cmd is selection pattern or ''
        if g:LookupFile_TagExpr == ''
            echomsg "not found file tag!"
            return
        endif
        if a:cmd != ''      " selection pattern
            call inputsave()
            let pattern = input("@", a:cmd)
            call inputrestore()
        endif
        exec 'LUTags ' . pattern
        return
...
func! CCTreeUsagePrompt() "{{{
    exec "silent! messages clear"
    exec "redraw"
    echomsg "1. <C-\>       [a|c|d|e|f|g|i|s|t]"
    echomsg "2. <C-\>f      tab, cursor file > lookup"
    echomsg "4. <C-\>]f     horizontally <C-\>f"
    echomsg "3. <C-\>[f     vertically <C-\>f"
    echomsg "4. <C-\>F      only use lookup (support visual mode)"
    echomsg "5. <C-\><C-\>f  find file using selection register as pattern"
    echomsg "6. <C-\>e      search cursor word"
    echomsg "7. <C-\>E      search cursor word with confirm(support visual mode)"
    echomsg "8. <C-\><C-\>e  grep word using selection register as pattern"
endfunc "}}}


"局部跳转到赋值或定义处:gd, cs a 是去全局
"" current buffer
nmap <unique> <silent> <C-\>h :call CCTreeUsagePrompt()<CR>
